...
namespace tag_loader
{
	...
	datum tag_module_loader::tag_module_load(std::string tag_module_rel_path, int target_index)
	{
		...
		if (my_loader->isopened())
		{
			...
			s_sound_group_definition out_snd_tag;
			...
			for (int i = 0; (i < module_tag_table->size / 0x10); i++)
			{
				...
				//snd! tag index fixes
				if (tagtables_new_element[i].type.as_int() == 'snd!')
				{
					s_sound_group_definition*t_snd_tag = (s_sound_group_definition*)(tags::get_tag_data() + tagtables_new_element[i].data_offset);

					if (t_snd_tag->playback_index != (short)-1)
						t_snd_tag->playback_index += out_snd_tag.playback_index;
					if (t_snd_tag->first_pitch_range_index != (short)-1)
						t_snd_tag->first_pitch_range_index += out_snd_tag.first_pitch_range_index;
					if (t_snd_tag->scale_index != (unsigned __int8)-1)
						t_snd_tag->scale_index += out_snd_tag.scale_index;
					if (t_snd_tag->promotion_index != (unsigned __int8)-1)
						t_snd_tag->promotion_index += out_snd_tag.promotion_index;
					if (t_snd_tag->custom_playback_index != (unsigned __int8)-1)
						t_snd_tag->custom_playback_index += out_snd_tag.custom_playback_index;
					if (t_snd_tag->extra_info_index != (short)-1)
						t_snd_tag->extra_info_index += out_snd_tag.extra_info_index;
				}
	...
	datum tag_module_loader::tag_get_loaded_datum(blam_tag group,std::string tag_name)
	{
		for (auto& it = loaded_tag_list.begin(); it != loaded_tag_list.end(); it++)
		{
			if (_strnicmp(tag_name.c_str(), it->debug_tag_name.c_str(), 256) == 0)
				if (group == it->tag_instance.type)
					return it->tag_instance.datum_index;
		}
		return datum::Null;
	}
