...
/*********************************************************************
* Modified implementation of tag_block to suit the internal structures utilized by the game
...
struct h2_memory_array
{
	...
	{
		return (void*)MemoryBlockPointer;
	}
	...
	{
		memcpy(MemoryBlockPointer + MemoryBlockCount, arg0, GetFieldSize());
		MemoryBlockCount++;
	}
	//* Adds a range of elements from an array
	void AddRange(T* arg0, int count)
	{
		memcpy(MemoryBlockPointer + MemoryBlockCount, arg0, GetFieldSize()*count);
		MemoryBlockCount += count;
	}
	...
	{
		T* old_alloc = MemoryBlockPointer;
		T* new_alloc = new T[new_count];

		if (old_alloc != nullptr)
			memcpy_s(new_alloc, new_count * GetFieldSize(), old_alloc, GetTotalSize());

		if (new_count < MemoryBlockCount)
			MemoryBlockCount = new_count;

		MemoryBlockPointer = new_alloc;
