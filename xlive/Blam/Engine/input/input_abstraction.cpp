...
void input_abstraction_get_controller_preferences(e_controller_index controller_index, s_gamepad_input_preferences* preferences)
{
	INVOKE_TYPE(0x61BF4, 0x0, void(__cdecl*)(e_controller_index, s_gamepad_input_preferences*), controller_index, preferences);
}

void input_abstraction_handle_device_change(uint32 flags)
{
	INVOKE_TYPE(0x61C72, 0x0, void(__cdecl*)(int), flags);
}
...
void input_abstraction_apply_raw_mouse_update(e_controller_index controller, s_game_input_state* input_state)
{
	...
	if (H2Config_raw_input)
	{
		...
		input_state->mouse.yaw = time->seconds_per_tick * (float)mouse_state->lX * -(H2Config_raw_mouse_scale / 100);
		input_state->mouse.pitch = time->seconds_per_tick * (float)mouse_state->lY * -(H2Config_raw_mouse_scale / 100);
...
void __cdecl input_abstraction_update()
{
	...
	{
		if (input_globals->gamepad_states[controller].connected)
	...
	if (any_gamepad_connected)
	{
			...
			controller = next_controller(controller))
		{
			...
			//if (g_check_counter >= 0 && g_check_counter <= 20)
			//{
			//	LOG_CRITICAL_FUNC("controller {}", controller);
			//	g_check_counter++;
			//}
...
{
	uint32 result = INVOKE_TYPE(0x5E501, 0x0, uint32(__thiscall*)(s_input_abstraction_globals*, e_button_functions, void*), input_abstraction_globals, button, a3);
	/*	if (g_check_counter >= 0 && g_check_counter <= 20)
		{
			LOG_CRITICAL_FUNC("button {} : {} ",button,result);
			g_check_counter++;
		}*/
	return result;
}
