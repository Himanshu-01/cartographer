...
static void setup_game_options_text(const wchar_t* reports_path)
{
	...
	if (!error && file != NULL && game_is_active())
	{
		...
		fwprintf(file, L"%u\n", game_options->machine_options.valid_machine_mask);

		fwprintf(file, L"Machine Identifiers: ");
		for (size_t i = 0; i < 17; i++)
		{
			print_array_to_file(file, game_options->machine_options.machines[i].machine_identifier, 6);
		}
		...
		print_array_to_file(file, game_options->machine_options.local_machine_identifier.machine_identifier, 6);
